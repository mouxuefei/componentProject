...
//                lp.topMargin -= statusBarHeight;
     ...
     * 判断是否有网络
     */
    public static boolean isHasInterNetWork(Context context) {
    ...
    public static boolean isName(String name) {
        if (name.matches("[a-zA-Z\\u4e00-\\u9fa5]{1,20}")) {
            return true;
        } else {
            return false;
        }
    ...
    public static String formatYearAndMonthAndDay(Date date) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy年MM/dd");
        return format.format(date);
    }

    public static String formatYearAndMonthAndDayAndHAndMAndS(long date) {
        Date date1 = new Date();
        date1.setTime(date);
